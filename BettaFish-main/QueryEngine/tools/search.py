...
class TavilyNewsAgency:
    ...
    def process_search_results(raw_results):
        ...
        for result in raw_results:
            # 1. 验证来源可信度（示例：优先可信域名）
            trusted_domains = ["gov.cn", "edu.cn", "org.cn", "baidu.com", "sogou.com"]
            domain = result.get("url", "").split("//")[-1].split("/")[0]
            is_trusted = any(domain.endswith(d) for d in trusted_domains)

            # 2. 提取发布时间，优先近期信息（减少过时信息导致的幻觉）
            publish_time = result.get("publish_time", "")
            time_score = 1 if "2024" in publish_time or "2025" in publish_time else 0.5

            # 3. 综合评分（0-1）
            confidence_score = (0.7 * is_trusted) + (0.3 * time_score)

            processed_result = {
                **result,
                "confidence_score": confidence_score,
                "source_verified": is_trusted,
                "warning": "低可信度来源" if confidence_score < 0.6 else None
            }
            processed.append(processed_result)
        # 过滤极低可信度结果（阈值可配置）
        return [r for r in processed if r["confidence_score"] > 0.3]
# --- 3. 测试与使用示例 ---
