...
class ChapterGenerationNode(BaseNode):
    ...
    def _sanitize_chapter_blocks(self, chapter: Dict[str, Any]):
        ...
        def walk(blocks: List[Dict[str, Any]] | None):
            ...
            for block in blocks:
                ...
                # 马欢欢新添加的
                # 如果是paragraph块，且有来源数据，追加来源标注
                if block.get("type") == "paragraph" and hasattr(self, "sources") and self.sources:
                    # 提取段落现有文本
                    existing_text = self._extract_block_text(block)
                    # 生成来源标注
                    source_links = [f"[{i + 1}]({s['url']})" for i, s in enumerate(self.sources[:3])]
                    source_note = f"\n\n【信息来源】：{', '.join(source_links)}"
                    # 追加到段落的inlines中
                    inlines = block.get("inlines", [])
                    inlines.append(self._as_inline_run(source_note))
    ...
    def _normalize_table_cell(self, cell: Any) -> Dict[str, Any] | None:
        ...
        if cell is None:
            ...
            # 马欢欢新添加的
            return {"blocks": [self._as_paragraph_block("", self.sources)]}  # 传入来源
        if isinstance(cell, dict):
            ...
            # 马欢欢新添加的
            blocks = [self._as_paragraph_block(str(cell), self.sources)]  # 传入来源
        else:
            normalized = {}
            blocks = [self._as_paragraph_block(str(cell))]
    ...
    def _coerce_list_item(self, item: Any) -> List[List[Dict[str, Any]]]:
        ...
        if isinstance(item, list):
            ...
            # 马欢欢新修改的代码
            result.append([self._as_paragraph_block(item, self.sources)])  # 传入来源
        elif isinstance(item, (int, float)):
            # 马欢欢先把这行代码注释掉
            # result.append([self._as_paragraph_block(str(item))])
            # 马欢欢新修改的代码
            result.append([self._as_paragraph_block(str(item), self.sources)])  # 传入来源
        return result
    ...
    def _normalize_widget_block(self, block: Dict[str, Any]):
        ...
        # ========== 新增逻辑：替换原有空data初始化，针对line图表补充默认数据 ==========
        # 1. 先初始化基础data结构（替代原有 block["data"] = {"labels": [], "datasets": []}）
        block.setdefault("data", {})
        widget_data = block["data"]
        widget_subtype = block.get("subtype")  # 获取图表类型（line/bar/pie等）

        # 2. 仅对line类型图表，强制补充labels和datasets（核心新增）
        if widget_subtype == "line":
            # 2.1 补充labels（x轴：默认最近7天日期，优先用真实数据）
            if "labels" not in widget_data or not widget_data["labels"]:
                # 生成默认最近7天日期（格式：YYYY-MM-DD）
                default_labels = [(datetime.now() - timedelta(days=i)).strftime("%Y-%m-%d") for i in range(6, -1, -1)]
                # 尝试从self.sources提取真实日期（如果有）
                real_labels = []
                if hasattr(self, "sources") and self.sources:
                    for source in self.sources:
                        # 注意：这里的"sentiment_date"需匹配你实际的日期字段名，没有则用默认
                        if "sentiment_date" in source and source["sentiment_date"] not in real_labels:
                            real_labels.append(source["sentiment_date"])
                # 优先用真实日期，没有则用默认
                widget_data["labels"] = sorted(real_labels[-7:]) if real_labels else default_labels
                logger.info(f"Line图表labels缺失，已补充：{widget_data['labels']}")

            # 2.2 补充datasets（y轴：默认情感数据，优先用真实数据）
            if "datasets" not in widget_data or not widget_data["datasets"]:
                # 默认情感数据（正面/负面占比，示例值）
                default_datasets = [
                    {
                        "label": "正面情感占比（%）",
                        "data": [35, 42, 48, 51, 55, 58, 62],  # 对应7天labels
                    ...
                    {
                        "label": "负面情感占比（%）",
                        "data": [25, 22, 18, 15, 12, 10, 8],  # 对应7天labels
                        "borderColor": "#e74c3c",  # 红色
                        "borderWidth": 2,
                        "fill": False
                    }
                ]
                # 尝试从self.sources提取真实情感数据（如果有）
                real_positive = []
                real_negative = []
                if hasattr(self, "sources") and self.sources:
                    for source in self.sources:
                        # 注意："positive_ratio"/"negative_ratio"需匹配你实际的情感字段名
                        if "positive_ratio" in source:
                            real_positive.append(source["positive_ratio"])
                        if "negative_ratio" in source:
                            real_negative.append(source["negative_ratio"])
                # 优先用真实数据，没有则用默认
                if real_positive and real_negative:
                    default_datasets[0]["data"] = real_positive[-7:]  # 取最近7个真实正面数据
                    default_datasets[1]["data"] = real_negative[-7:]  # 取最近7个真实负面数据
                widget_data["datasets"] = default_datasets
                logger.info(f"Line图表datasets缺失，已补充默认/真实数据")

        # ========== 原有逻辑：确保widget基础配置（完全保留） ==========
        block.setdefault("props", {})
        if block["props"].get("width") is None:
            block["props"]["width"] = "100%"
        if block["props"].get("height") is None:
            block["props"]["height"] = 400
