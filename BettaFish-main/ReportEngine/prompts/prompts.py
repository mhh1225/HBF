...
**链接保留强制规则（必须严格遵守，否则视为不合格报告）：**
1. 所有来自三个引擎报告（query_engine_report、media_engine_report、insight_engine_report）中的事实、数据、用户评论，其附带的来源链接必须完整保留，格式为「（来源：[平台名称](链接地址)）」，不得删除、修改链接地址或平台名称；
2. 链接必须渲染为可点击状态，使用蓝色（#0066CC）、下划线样式，点击后跳转至对应页面，不得转为纯文本；
3. 正确渲染格式参考：“中国农业大学推免比例60%（来源：[知乎](https://www.zhihu.com/answer/987654321)）”，其中“https://www.zhihu.com/answer/987654321”为Insight Engine传递的数据库真实链接，实际渲染时需使用该原始值，不得替换；
4. 若原始内容有链接而HTML报告中未保留或不可点击，需重新生成该章节内容。
5. 严禁将Insight Engine传递的真实链接替换为任何示例链接（如`https://www.zhihu.com/answer/123456`），必须直接使用`QueryResult.url`的原始值；若发现链接地址与Insight传递的真实值不一致，需重新渲染该章节；
6. 链接渲染时仅允许添加蓝色（#0066CC）、下划线样式，不得添加其他影响跳转的CSS样式（如`pointer-events: none`），确保点击后能正常跳转至数据库对应的真实页面。
7. 链接格式校验：
   - 抖音链接必须包含“/video/”（如https://www.douyin.com/video/xxx），禁止“/v.douyin.com/”短链；
   - 知乎链接必须包含“/question/”或“/answer/”；
   - 小红书链接必须包含“/explore/”且带xsec_token参数；
   - 不符合上述格式的链接，视为无效链接，需重新从数据库提取替换。
...
16. 所有事实、数据、用户评论对应的来源链接，必须直接使用Insight Engine传递的`QueryResult.url`原始值（该值为数据库对应表的真实链接，如小红书`xhs_note.note_url`、抖音`douyin_aweme.aweme_url`），格式严格保留为「（来源：[平台名称](真实链接)）」，不得修改链接地址、平台名称或调整格式；
17. 链接相关文本（含“（来源：[平台名称](真实链接)）”完整片段）不得拆分到多个`inline`节点，必须作为一个完整单元存入`paragraph.inlines`，确保后续HTML渲染时能识别为可点击链接；
18. 若`QueryResult.url`为“未获取到该内容的来源链接”，则严格标注为「（无有效来源链接）」，不得自行补充任何链接地址。
