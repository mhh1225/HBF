...
class Research:
    ...
    # 马欢欢新增：跟踪去重后的搜索关键词（核心事件）
    unique_core_queries: set = field(default_factory=set)  # 去重的核心事件关键词
    # 马欢欢新增：记录搜索次数（按核心事件去重后）
    core_search_count: int = 0  # 有效搜索次数
    def add_search(self, search: Search):
        """添加搜索记录"""
        self.search_history.append(search)
        # 马欢欢新增
        # 提取核心关键词（简化处理：取查询的核心短语，可根据实际需求优化）
        core_query = self._extract_core_query(search.query)
        if core_query not in self.unique_core_queries:
            self.unique_core_queries.add(core_query)
            self.core_search_count += 1  # 仅在新核心事件时增加计数
    def _extract_core_query(self, query: str) -> str:
        """提取核心关键词（例如：从"转基因玉米试验田被毁事件 最新消息"中提取核心）"""
        # 可根据实际业务优化关键词提取逻辑
        stop_words = ["事件", "最新", "消息", "情况", "是否", "真的", "吗", "？", " "]
        core = query.lower().strip()
        for word in stop_words:
            core = core.replace(word, "")
        return core if core else query.lower().strip()
    # 马欢欢新增结束
