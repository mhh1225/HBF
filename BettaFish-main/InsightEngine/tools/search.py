...
class DBResponse:
    ...
    # 马欢欢新增metadata字段（加在这里即可）
    metadata: Optional[Dict[str, Any]] = None  # 可选字典类型，默认值None
    ...
    def search_topic_globally(self, topic: str, limit_per_table: int = 100) -> DBResponse:
        ...
        for table, config in search_configs.items():
            ...
            for row in raw_results:
                ...
                # 马欢欢新增：按平台精准匹配URL字段（和数据库表严格对应）
                # 严格按数据库表-字段对应关系读取链接，不做任何修改
                platform_short = table.split('_')[0]
                if platform_short == 'xhs':
                    content_url = row.get('note_url', '未获取到该内容的来源链接')  # 小红书→xhs_note.note_url
                elif platform_short == 'douyin':
                    content_url = row.get('aweme_url', '未获取到该内容的来源链接')  # 抖音→douyin_aweme.aweme_url
                elif platform_short == 'weibo':
                    content_url = row.get('note_url', '未获取到该内容的来源链接')  # 微博→weibo_note.note_url
                elif platform_short == 'zhihu':
                    content_url = row.get('content_url', '未获取到该内容的来源链接')  # 知乎→zhihu_content.content_url
                elif platform_short == 'kuaishou':
                    content_url = row.get('video_url', '未获取到该内容的来源链接')  # 快手→kuaishou_video.video_url
                elif platform_short == 'bilibili':
                    content_url = row.get('video_url', '未获取到该内容的来源链接')  # B站→bilibili_video.video_url
                elif platform_short == 'tieba':
                    content_url = row.get('note_url', '未获取到该内容的来源链接')  # 贴吧→tieba_note.note_url
                else:
                    content_url = '未获取到该内容的来源链接'

                # 禁止短链和无效链接格式
                if content_url.startswith('https://v.douyin.com/') or content_url.startswith('https://v.kuaishou.com/'):
                    content_url = '未获取到该内容的来源链接'
                # 马欢欢新增结束
                all_results.append(QueryResult(
                    ...
                    url=content_url,  # 马欢欢这里用新定义的content_url，替代原来的长串row.get
    ...
    def search_topic_on_platform(
        ...
        for config in platform_configs:
            ...
            for row in raw_results:
                ...
                # 马欢欢新增：按平台精准匹配你数据库的对应链接字段
                platform_short = platform  # 这里的platform是函数传入的指定平台（如douyin、xhs）
                if platform_short == 'xhs':
                    content_url = row.get('note_url', '未获取到该内容的来源链接')  # 小红书→你数据库的xhs_note.note_url
                elif platform_short == 'douyin':
                    content_url = row.get('aweme_url', '未获取到该内容的来源链接')  # 抖音→你数据库的douyin_aweme.aweme_url
                elif platform_short == 'weibo':
                    content_url = row.get('note_url', '未获取到该内容的来源链接')  # 微博→你数据库的weibo_note.note_url
                elif platform_short == 'zhihu':
                    content_url = row.get('content_url',
                                          '未获取到该内容的来源链接')  # 知乎→你数据库的zhihu_content.content_url
                elif platform_short == 'kuaishou':
                    content_url = row.get('video_url', '未获取到该内容的来源链接')  # 快手→你数据库的kuaishou_video.video_url
                elif platform_short == 'bilibili':
                    content_url = row.get('video_url', '未获取到该内容的来源链接')  # B站→你数据库的bilibili_video.video_url
                elif platform_short == 'tieba':
                    content_url = row.get('note_url', '未获取到该内容的来源链接')  # 贴吧→你数据库的tieba_note.note_url
                else:
                    content_url = '未获取到该内容的来源链接'
                ...
                if content_url.startswith('https://v.douyin.com/') or content_url.startswith('https://v.kuaishou.com/'):
                    content_url = '未获取到该内容的来源链接'
                # 马欢欢结束
                all_results.append(QueryResult(platform=platform, content_type=config['type'], title_or_content=content if content else '', author_nickname=row.get('nickname') or row.get('user_nickname'), url=content_url,  publish_time=self._to_datetime(time_key), engagement=self._extract_engagement(row), source_keyword=row.get('source_keyword'), source_table=table))
