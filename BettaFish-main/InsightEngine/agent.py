...
# 马欢欢新导入的类
from .state import State, Search  # 补充Search导入
...
class DeepSearchAgent:
    ...
    def execute_search_tool(self, tool_name: str, query: str, paragraph_index: int,**kwargs) -> DBResponse:
        """
        ...
        # 马欢欢新增
        # 获取当前段落的研究状态
        current_paragraph = self.state.get_paragraph(paragraph_index)
        if not current_paragraph:
            raise ValueError(f"段落索引 {paragraph_index} 不存在")
        research = current_paragraph.research

        # 检查是否已达到3次有效搜索
        if research.core_search_count >= 3:
            logger.info(f"  ❌ 核心事件已搜索3次，停止继续搜索（query: {query}）")
            return DBResponse(
                tool_name=tool_name,
                parameters={"query": query,
                            # "search_count": research.core_search_count,  # 新增：记录当前搜索次数
                            **kwargs},
                results=[],
                results_count=0,
                metadata={"message": "已达到最大搜索次数（3次），未发现有效记录"}
            )
        # 马欢欢结束
        ...
        integrated_response = DBResponse(
            ...
            metadata = {"search_count": research.core_search_count}  # 马欢欢新增：记录当前搜索次数
        )
        # 马欢欢新增
        # 将搜索结果添加到状态（关键：触发Research的add_search方法更新计数）
        for result in unique_results:
            search = Search(
                query=query,
                # 直接访问QueryResult的类属性，同时做容错（避免属性不存在）
                url=result.url if hasattr(result, 'url') else "",
                # 注意：QueryResult的标题属性是title_or_content（从日志里的QueryResult实例能看到）
                title=result.title_or_content if hasattr(result, 'title_or_content') else "",
                content=result.title_or_content if hasattr(result, 'title_or_content') else "",
                score=result.score if hasattr(result, 'score') else None
            )
            research.add_search(search)
        # 检查是否需要进行情感分析
        enable_sentiment = kwargs.get("enable_sentiment", True)
    ...
    def _initial_search_and_summary(self, paragraph_index: int):
        ...
        search_response = self.execute_search_tool(
            search_tool,
            search_query,
            paragraph_index=paragraph_index,  # 新增参数
            **search_kwargs
        )
        # 转换为兼容格式
        search_results = []
    ...
    def _reflection_loop(self, paragraph_index: int):
        ...
        for reflection_i in range(self.config.MAX_REFLECTIONS):
            ...
            # 马欢欢新增：关键：获取当前段落的research对象
            research = paragraph.research
            # 生成反思搜索查询
            # 马欢欢先把这行给注释掉
            # reflection_output = self.reflection_node.run(reflection_input)
            # 马欢欢新修改好的
            # 关键：调用run时传入research参数（支持扩展其他kwargs）
            reflection_output = self.reflection_node.run(
                reflection_input,
                research=research,  # 传入research对象
                # 可扩展其他kwargs，比如LLM参数、自定义配置等
                # temperature=0.7,
                # max_tokens=2000
            )
