...
class ReflectionNode(BaseNode):
    ...
    def run(self, input_data: Any, **kwargs) -> Dict[str, str]:
        ...
        try:
            # 马欢欢新增
            # ========== 新增：终止条件判断 ==========
            # 从kwargs中获取research对象（核心：状态跟踪实例）
            research = kwargs.get("research")
            if research:
                logger.info(f"当前段落已执行 {research.core_search_count} 次核心搜索")
                # 1. 检查核心搜索次数是否达到3次
                if hasattr(research, "core_search_count") and research.core_search_count >= 3:
                    # 2. 检查所有搜索结果是否为空
                    all_empty = all(len(search.content.strip()) == 0 for search in research.search_history)
                    if all_empty:
                        self.log_info(f"✅ 3次搜索均无结果，判定为无真实记录")
                        research.mark_completed()  # 标记为完成，避免继续搜索
                        return {
                            "search_query": "",  # 返回空查询表示停止搜索
                            "reasoning": f"该事件经过3次搜索未发现任何真实记录，判定为无有效信息，停止搜索"
                        }
            # 马欢欢结束
            if not self.validate_input(input_data):
