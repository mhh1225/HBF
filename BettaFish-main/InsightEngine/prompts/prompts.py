...
7. 所有引用的事实、数据、用户评论，必须按“事实描述（来源：[平台名称](真实链接)）”格式标注链接：
   - 真实链接必须来自搜索结果`QueryResult.url`字段（数据库对应表的真实链接，如知乎`zhihu_content.content_url`、抖音`douyin_aweme.aweme_url`）；
   - 若`QueryResult.url`为“未获取到该内容的来源链接”，则标注为“事实描述（无有效来源链接）”；
   - 正确示例（必须与数据库字段格式一致）：
    - 知乎：“用户评价‘中农科研强’（来源：[知乎](https://www.zhihu.com/question/416640491/answer/3378925664)）”；
    - 抖音：“网友点赞校长拨穗（来源：[抖音](https://www.douyin.com/video/7546590128819424550)）”；
    - 小红书：“考生关注宠物专业（来源：[小红书](https://www.xiaohongshu.com/explore/67fe9cbf000000001b03f901?xsec_token=ABnVD2Vm0uKQrhaN5edmWZMKnTOxF0jIlslPB92PA-6ws=&xsec_source=pc_search)）”；
    - 无链接：“某网友吐槽食堂（无有效来源链接）”；
   - 严禁使用示例中的占位符链接（如`https://www.zhihu.com/answer/123456789`），必须使用搜索结果中的真实链接，否则该内容视为无效。
